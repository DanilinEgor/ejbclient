...
public class ShowServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		response.getWriter().write("<a href=\"/Client/show?type=all\">All</a>");
		if (session.getAttribute("username") != null)
			response.getWriter().write(
					" | <a href=\"/Client/show?type=my\">My</a>");
		response.getWriter().write("<br><hr>");
		List<Post> posts = null;
		if (request.getParameter("type").equals("my"))
			posts = pb.getUserPosts((String) session.getAttribute("username"));
		else
			posts = pb.getAllPosts();
		for (Post post : posts) {
			if (Drawing.class.isAssignableFrom(post.getClass())) {
				...
				for (Shape shape : shapes) {
					...
					if (request.getSession().getAttribute("username") != null
							&& request.getSession().getAttribute("username")
									.equals(dr.getAuthor()))
						response.getWriter().write(
								"  | <a href=\"/Client/delete?type=shape&id="
										+ shape.getId() + "\">Remove</a>");
				...
				if (request.getSession().getAttribute("username") != null
					...
					response.getWriter()
							.write("Add shape:<br><form method=\"post\" action=\"/Client/add\">"
									...
									+ "xC:<input name=\"xc\"><br>"
									+ "yC:<input name=\"yc\"><br>"
									+ "radius:<input name=\"radius\"><br><br>"
									+ "Rectangle:<br>"
									+ "x:<input name=\"x\"><br>"
									+ "y:<input name=\"y\"><br>"
									+ "w:<input name=\"w\"><br>"
									+ "h:<input name=\"h\"><br><br>"
									+ "Line:<br>"
									+ "x1:<input name=\"x1\"><br>"
									+ "y1:<input name=\"y1\"><br>"
									+ "x2:<input name=\"x2\"><br>"
				...
				if (!comments.isEmpty()) {
					...
					for (Comment comment : comments) {
						...
						response.getWriter()
								.write("Mark: " + comment.getMark());
						if (request.getSession().getAttribute("username") != null
								&& request.getSession()
										.getAttribute("username")
										.equals(comment.getAuthor()))
							response.getWriter().write(
									"  | <a href=\"/Client/delete?type=comment&id="
											+ comment.getId() + "\">Remove</a>");
				...
				if (request.getSession().getAttribute("username") != null)
					response.getWriter()
							.write("Add comment:<br><form style=\"display:block; width:200px; margin-left:100px;\" method=\"post\" action=\"/Client/add\">Text:<input name=\"text\"><input type=\"hidden\" name=\"postType\" value=\"comment\"><input type=\"hidden\" name=\"drawingID\" value="
									+ dr.getId()
									+ ">"
									+ "<br><input type=submit value=\"Add\"></form><br>");
				if (request.getSession().getAttribute("username") != null
						&& request.getSession().getAttribute("username")
								.equals(dr.getAuthor()))
					response.getWriter().write(
							"  | <a href=\"/Client/delete?type=drawing&id="
									+ dr.getId() + "\">Remove</a>");
